...
# AERSURFACE known-answer comparisons, the AERSCREEN reference runs,
# the CLI run smoke tests). Those SKIP silently when the binaries are
# not on PATH, so a green `make test-full` says nothing about them --
# build them with scripts/build_aermod.sh, build_bpip.sh,
# build_aersurface.sh and build_aerscreen.sh, then use this target,
